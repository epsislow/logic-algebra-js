dg = {
  'ev': function() {
    var pub = {
    'n': {},
    'cr': function (name, evData) {
      this.n[name] = new CustomEvent(name, evData);
      return this.n;
    },
    'addLis': function (name, proc) {
      addList(name, proc);
    },
    'disp': function(name) {
      dispEv(name);
    ...
    pub.t = document.createTextNode(null);
    var addList = this.t.addEventListener.bind(t);
    var dispEv = this.t.dispatchEvent.bind(t);
  ...
  'intx':async function(array1, array2) {
  ...
  for (i = 0; i < list.length; i += 1) {
    ...
    if (node.parentId !== "0") {
	  ...
	  'reduceArb':async function (list, res = {}) {
	    for(var i in list) {
	      if(!(i in res)) {
	        if(!list[i].length) {
	          res[i] = 0;
	  ...
	  'arbNodes':async function (list,type = 0) {
		if (type == 0) {
	  ...
	  'delSum':async function(k) {
        delete this.sums[k];
      },
	  'getSum':async function(k) {
	    return this.sums[k];
	  },
    'del':async function(k) {
      delete this.m[k];
    },
    'add':async function(k, v) {
      this.m[k] = v;
      return this;
    },
    ...
    'w2b':async function(w) {
      return w.split('');
    },
    'b2w':async function(b) {
      return b.join('');
    },
    'addWs':async function(k,w) {
      this.add(k, this.w2b(w));
      return this;
    },
    'getWs':async function(k) {
      return this.b2w(this.get(k));
    },
    'getSubBs':async function(k, start, len) {
      var v =[];
      var vs = this.get(k);
      for(var i in vs) {
    ...
    'chgSubBs':async function(k, start, nvs) {
      var vs = this.get(k);
      var vss = [];
      for(var i in vs) {
    ...
    'getNextCavl':async function() {
      this.cAvl++;
      return 'C' + this.cAvl;
    },
    'genCvalAll':async function(ckey, vslen) {
      //var vs;
      //if(Array.isArray(x)) {
     //   vs = this.get(k1);
     // }
    ...
    'cAvl': 0,
    'toObj':async function(names, values) {
      var res = {};
      for (var i = 0; i < names.length; i++)
        res[names[i]] = values[i];
      return res;
    },
    'pushOnce':async function(arr, item) {
      if (!arr.includes(item)){
        arr.push(item);
      } 
      return arr;
    },
    'unique':async function (a1) {
      var r = [];
      for (var i in a1) {
    ...
    'concatUnique':async function(a1, a2) {
      var r = [];
      for (var s in a1) {
        if (!r.includes(a1[s])) {
          r.push(a1[s]);
	...
	'objEmpty':async function (a, useJq = 1) {
		if(useJq) {
  ...
  'sh':{
	...
	'shortB':async function (x, optimize = 1, convB = 1, d=0) {
	  var vars;
	  ...
	  if (!vars.length) {
		...
		try {
			var f =async Function('return (('+ x + ') >>> 0) & 1')();
			if (f === '') {
				throw 'hopa';
			}
			return f+'';
	...
	//	r[t] = this.execr(st);
	  ...
	  try {
		r[t]async=Function('return (('+st + ') >>> 0) & 1')() + '';
	  } catch (e) {
...
//	    }
	...
	'convBr2B':async function(br, not) {
	  var bor =[];
	  var band;
	  for(var i in br) {
	    band = [];
	...
	'convBrJoinPush':async function(br, opr) {
	  var ust;
	  var hasv = false;
	  var hasr = false;
	  for(var i in br) {
	...
	'hasBrackets':async function(ust) {
	  return (ust.charAt(0)== '(' && ust.charAt(ust.length-1)) == ')';
	},
	'hasOpB':async function(ust) {
	  hasv = ((ust+'').match(/[\\&\\|\\^]/g));
	  if (hasv) {
	    hasv = hasv.length > 0;
	  };
	...
	'optimizeShortB':async function (table, debug=0) {
	  var table0 = table;
	var table2;
	var otbl = [];
	var txt = [];
	...
	'findSameValues':async function(data,datb, oneValueDiff = false, allowNotSameKeys = true) {
  ...
  'findIfSameKeys':async function(data,datb) {
    for(var k in data) {
      if(!(k in datb)) {
        return false;
      }
	...
	'repl':async function (x,replacers, wExec =0) {
		var vs;
		if(Array.isArray(x)) {
	      vs = x;
	    } else {
	...
	'replB':async function (x, replacers, d =0) {
		var r = x + '';
	...
	'execr':async function (x, checkVars=0, pos = 0, lvl = 0,) {
	  var l;
	  var params = [];
	  var op = '';
	  var not = 0;
	...
	'hasVarInSum':async function(vsx, needle) {
	  var val;
	  for(var k in vsx) {
	    for(var i in vsx[k]) {
	      val = vsx[k][i];
	...
	'getVarBitsInV':async function (v) {
	  var varBits = [];
	...
	'noOfVarsB':async function(x) {
	  return this.hasVarB(x,1);
	},
	'simplifyVarsName':async function (v) {
		var r =	v.replaceAll
		  (/([a-z][^\,\[\]\{\}\+\&\\|\\^\\~\\)\\(]*)/ig, '$&#');
	...
	'hasVarB':async function (v, which = 0) {
	  var r = v.match
	    (/([a-z][^\&\\|\\^\\~\\)\\(]*)/ig);
	  if (which) {
	...
	'getValuesForAllKeys':async function(datas, keys) {
	  var key = keys.shift();
	  if (key == undefined) {
	    return datas;
	  }
	...
	'getValuesForKey'async:function(data, key) {
	  var newdatas = [];
	  var newdata = data;
	  newdata[key] = '0';
	...
	'carryForm'async:function(sum) {
	  
	},
	'sumForm':async function (sum) {
	...
	'sumch':async function (sumName, ks, doit = false, debug= 0) {
		var vslen = ks.length;
		var vs = [];
	...
	'sum':async function(k1,k2, short=0, debug = 0) {
	    var vs1,vs2;
	    if(Array.isArray(k1)) {
	      vs1 = k1;
	    } else {
