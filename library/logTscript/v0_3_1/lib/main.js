...
/**
 * Assign contiguous level indices (0, 1, 2, ...) to a repeat tree,
 * and validate the 256 iteration limit per nesting group.
 */
...
/**
 * Expand a text node, deduplicating based on referenced levels.
 * Lines that only reference a subset of levels are emitted once
 * per unique combination of those levels.
 */
function expandTextNode(content, levelValues, seenMap) {
  // If we are outside any repeat (no level values), just return content as-is
  if (levelValues.length === 0) return content;

  // Split content into lines, process each line independently
  // (a text node may contain multiple lines)
  ...
  for (let li = 0; li < lines.length; li++) {
    ...
    // If line has no ? references at all, emit as-is (but only once per unique content)
    if (!refs.hasBare && refs.levels.size === 0) {
      // No placeholders – deduplicate: emit only on first encounter
    ...
    // Determine effective referenced levels
    let effectiveLevels;
    if (refs.hasBare) {
      // bare ? triggers all levels
      effectiveLevels = [];
    ...
    // Build dedup key from referenced levels' current values
    const keyParts = effectiveLevels.map(l => levelValues[l] !== undefined ? levelValues[l] : '?');
    const dedupKey = 'ref:' + line + ':' + keyParts.join(',');

    if (seenMap[dedupKey]) {
      // Already emitted this combination – skip
      continue;
    }
    seenMap[dedupKey] = true;

    // Compute sequential counter for bare ?
    // The seqCounter is per-template (per unique line template)
    let expanded = line;

    // Replace ?N (longest match first: ?10 before ?1)
    ...
    // Replace bare ? with sequential counter
    if (refs.hasBare) {
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { preprocessRepeat, parseRepeatTree, findRepeatKeyword, Token, Tokenizer };
}
...
parseLoad() {
  ...
  // Parse loaded file in isolation (preprocess repeat blocks)
...
parsePcbDefinition() {
  ...
  while (this.c.type !== 'EOF') {
    ...
    // Track current line (blank lines no longer terminate the block;
    // only ':' or EOF ends a pcb definition block)
    lastLine = this.c.line;
