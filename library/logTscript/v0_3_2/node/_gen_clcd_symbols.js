/**
 * Generate devices/clcd-symbols_generated.js from fa_index_generated.json.
 * Run: node node/_gen_fa_index.js && node node/_gen_clcd_symbols.js
 */
const fs = require('fs');
const path = require('path');
const { ROOT, RES_FONTS } = require('./js/paths');

const INDEX_PATH = path.join(RES_FONTS, 'fa_index_generated.json');
const OUT_PATH = path.join(ROOT, 'devices', 'clcd-symbols_generated.js');
...
function main() {
  if (!fs.existsSync(INDEX_PATH)) {
    console.error('Missing', INDEX_PATH, '— run: node node/_gen_fa_index.js');
    process.exit(1);
  }
  ...
  const lines = [];
  lines.push('/* AUTO-GENERATED — Regenerate: node node/_gen_fa_index.js && node node/_gen_clcd_symbols.js */');
  lines.push('');
  ...
  lines.push('}');
  lines.push('');
  lines.push('var CLCD_FA_ICON_SIZE_DEFAULT = 22;');
  lines.push('');
  lines.push('var CLCD_CANVAS_NATIVE = {');
  lines.push('  digit7: { w: 28, h: 44 },');
  lines.push('  digit14: { w: 28, h: 44 },');
  lines.push('  dp: { w: 12, h: 8 },');
  lines.push('  colon: { w: 8, h: 32 },');
  lines.push('};');
  lines.push('');
  lines.push('function resolveClcdFaIconSize(sym) {');
  lines.push('  var sz = sym && sym.size;');
  lines.push('  return (sz !== undefined && sz !== null) ? sz : CLCD_FA_ICON_SIZE_DEFAULT;');
  lines.push('}');
  lines.push('');
  lines.push('function resolveClcdCanvasScale(sym, renderer) {');
  lines.push('  var nat = CLCD_CANVAS_NATIVE[renderer];');
  lines.push('  if (!nat) return 1;');
  lines.push('  var targetH = (sym && sym.size !== undefined) ? sym.size : nat.h;');
  lines.push('  return targetH / nat.h;');
  lines.push('}');
  lines.push('');
  ...
  lines.push('  module.exports.CLCD_CANVAS_NATIVE = CLCD_CANVAS_NATIVE;');
  lines.push('  module.exports.getClcdSymbolDef = getClcdSymbolDef;');
  lines.push('  module.exports.resolveClcdFaIconSize = resolveClcdFaIconSize;');
  lines.push('  module.exports.resolveClcdCanvasScale = resolveClcdCanvasScale;');
