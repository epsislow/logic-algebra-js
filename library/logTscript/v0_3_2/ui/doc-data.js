...
Apasă **Load & Run** în doc viewer (sau copiază în editor). Scriptul conține definițiile \`alu4\` + \`cpu4\`, instanța \`.cpu\`, \`probe\` pe ACC/PC/IR și **4 cicluri** de execuție.

În panoul din dreapta vezi **7seg** (ACC) și componentele mem/counter/reg. În consolă: liniile \`probe\` + \`show\` de la final.
...
\`\`\`logts-play
# Presupune că ai rulat deja exemplul complet (definițiile chip/board sunt în editor).
# Altfel, folosește mai întâi blocul „Exemplu rulabil” de mai sus.
...
- Teste automate: \`test_suite_ported.js\` (859–864)
