...
const DOC_SECTIONS = [
  ...
  },
  {
    ...
    items: [
      ...
      { file: 'comp-logic.md', label: 'Component logic (`comp [logic]`)', searchPrimary: 'logic comp.logic program block exec redirect query pin myX set on raise mutationFailed add remove', searchExtra: 'logts-play pipeline scoreIn modifier number text bool isJohnOwner johnOwns execCount doc(comp.logic) logic { + - } tombstone dynamic' },
      { file: 'logic-runtime.md', label: 'Logic runtime (static / dynamic / mutations)', searchPrimary: 'logic runtime mutation { + - } tombstone dynamic overlay mutationFailed assert retract ground fact text number bool wire', searchExtra: 'logts-play Load and Run static dynamic effective KB comp logic inline world query warehouse move box' },
      { file: 'logic-constraints.md', label: 'Logic constraints (<= validation)', searchPrimary: 'constraint <= validation mutationFailed init rollback slotAvailable singleLocation text number bool wire mutation', searchExtra: 'logts-play Load and Run proposed state OR relation capacity move atomic count/2' },
      { file: 'logic-indexing.md', label: 'Logic indexing (fact index & count/2)', searchPrimary: 'indexFacts indexRebuild full delta count/2 fact index logic indexing', searchExtra: 'logts-play Load and Run capacity delta idempotent indexFacts 0 mutationFailed' },
