...
var QueueComponent = class QueueComponent extends BuiltinComponent {
  static get type() { return 'queue'; }
  static get shortnames() { return { fifo: 'queue' }; }
  ...
  createDevice(name, baseId, bits, attributes, initialValue, returnType, ctx) {
    ...
    if (width <= 0) throw Error(`Queue width must be positive for component ${name}`);
    if (length <= 0) throw Error(`Queue length must be positive for component ${name}`);
    const storageId = baseId;
    if (typeof addQueueStorage === 'function') {
      addQueueStorage({ id: storageId, width, length, mode: 'fifo' });
    }
    return { deviceIds: [storageId], ref: null };
  }

  getForbidDirectAssign() {
    return 'Cannot assign a value to a queue component. Use :push, :pop, :clear, and :set properties instead.';
  }

  _applyQueueOps(id, width, pending, reEvaluate, ctx) {
    ...
    if (pushInBlock && popActive) {
      throw Error('Conflicting queue operations');
    }
  ...
  applyProperties(comp, compName, pending, when, reEvaluate, ctx) {
    ...
    const id = comp.deviceIds[0];
    const width = this.getWidthBits(comp.attributes);
    this._applyQueueOps(id, width, pending, reEvaluate, ctx);
...
if (typeof module !== 'undefined' && module.exports) { module.exports = QueueComponent; }
