...
Prezența unui **`comp [cpu]`** în același fișier este **opțională**. Integrarea (RAM partajat, **`wait =`** stall) = sub-faza **5c**.
...
- CPU: **`ram = .data`** (sau `ram:` intern) — aceeași instanță `comp [mem]` ca în **`mems:`** al DMA.
- Ordinea scriptului sau stall-ul (mai jos) evită race-uri naive load/store vs copy.

#### Stall pe CPU — **`wait =`** (agreat; fără `dma = .dma`)

Un singur mecanism pe **`comp [cpu]`**:

| Atribut | Rol |
|---------|-----|
| **`wait =`** | Wire `1bit`, expresie sau pin — dacă activ (`1`) la **`set`** / **`run`**, CPU **stall** (PC neschimbat, fără step pe acel puls). |

**Regulă generală:** `wait` activ (`1`) = CPU **nu execută** instrucțiunea curentă. PC rămâne la ultima instrucțiune **complet terminată**.

##### `set = 1` (un pas)

| `wait` | Efect |
|--------|--------|
| `1` | **Stall** — 0 instrucțiuni, PC neschimbat |
| `0` | Exact **un** `cpuStep` |

##### `run = 1` + `maxSteps` (buclă)

Înainte de **fiecare** instrucțiune din buclă (re-evaluare `wait` în **wave**):

```
executed = 0
while !halted && executed < maxSteps:
    if wait == 1:
        break              # oprire la primul hold — fără step pe acest ciclu
    cpuStep()
    executed += 1
```
...
| Aspect | Semnificație |
|--------|----------------|
| **`maxSteps`** | Maxim instrucțiuni **efectiv executate** în **acest** puls `run` (stall **nu** consumă din buget) |
| **`wait = 1` la intrare** | 0 pași, PC neschimbat |
| **`wait = 1` după N pași** | Bucla **se oprește** (early exit); PC = după a N-a instrucțiune |
| **După ce `hold` revine la 0** | CPU **nu** continuă singur — trebuie **puls nou** `run = 1` (sau `set`) |
| **Fără auto-reluare** | Un singur `run` nu „așteaptă” DMA în buclă; oprește la primul hold |

**Exemplu temporal:**

```logts
1wire hold = .dma:busy
.u:{ run = 1 }          # maxSteps: 100 — execută până la primul hold sau HALT
# ... DMA busy → hold=1 → run se oprește după N pași
# ... DMA done → hold=0 — CPU stă pe PC, nu rulează singur
.u:{ run = 1 }          # continuă de la PC, alt buget maxSteps
```

**Nu:** spin în `while` până `wait=0` în același `applyProperties` (risc blocare / semantica neclară în legacy).

**DMA, network, keyboard, etc.:** utilizatorul le agregă în script — **nu** există `dma = .dma`, `network =`, … pe CPU.

```logts
comp [mem] .ram:
  depth: 8
  length: 256
  ports: 2          # opțional avansat: CPU port 1, DMA port 2
  on: 1
  :
...
# Toate condițiile de așteptare într-un singur wire:
1wire hold = OR(.dma:busy, .net:rxEmpty)
...
| Scenariu | `hold` |
|----------|--------|
| Doar DMA | `1wire hold = .dma:busy` |
| DMA + network | `1wire hold = OR(.dma:busy, .net:rxEmpty)` |
| Fără stall | omit `wait` sau `1wire hold = 0` |

**Wave:** `wait` poate fi fire re-evaluată în același step ca `set`/`run`.

**Nu în 5c:** `dma = .dma` sau alte binding-uri stall per componentă; **`WFI`** / sleep până la IRQ (opțional / script extern).

#### Mem multi-port (avansat, tot în 5c)

- **`comp [mem] ports: 2`** — CPU port 1 (`adr`/`get`/…), DMA acces pe port 2 (`2adr`/…) pe **aceeași** memorie fizică în același step (fără duplicate `.ram`).
- Coliziune: două scrieri aceeași adresă în același step → eroare (regulă existentă `mem`).

#### Livrabile 5c

- `cpu.js` + device: **`wait`** înainte de fiecare `cpuStep`; `cpuRun` **break** la primul `wait=1` (fără consum `maxSteps` pe stall)
...
## Faza 6: spațiu de adrese unificat (`comp [mmap]`) — **implementat**
