...
todos:
  ...
  - id: parse-def-body-choice
    ...
    status: pending
  - id: phase1-tests
    content: "Teste Faza 1: inline `ipv4?:`; def+`name?`; `ethernet` obligatoriu → eroare all-fail; `ethernet?` → 0 biți + footer; rollback"
    status: pending
  - id: lut-deferred-commit
    content: "Faza 2: în interpreter.js evalProtocolInvoke — buffer pendingLutEntries, flush LUT doar la parse success total"
    status: pending
  - id: rest-reserve-suffix
    content: "Faza 2: `rest -Nb` explicit; nice-to-have `rest -footer` dacă e fezabil"
    status: pending
  - id: nested-def-composition
    content: "Faza 2: def imbricate (vlan? → qinq?, ethernet → vlan? + ipv4?/arp?) + teste nesting"
    status: pending
  - id: decode-guard
    content: "Faza 1: în interpreter.js evalProtocolDecode — respinge `:decode()` pe protocoale cu tentative + test regresie"
    status: pending
  - id: parse-view-metadata
    content: "Faza 3: atribut opțional `parseView: tree` + ParseView registry + show tree; fără atribut = blob plat (neschimbat)"
    status: pending
  - id: parse-view-field-access
    content: "Faza 3: `parsed:typeA:dataA` — resolveParseViewPath + slice din blob (doar când parseView: tree)"
    status: pending
  - id: docs-tentative
    content: "Faza 4: protocol.md — invocare vs decode, mandatory/tentative, feature matrix, exemple runnable"
    status: pending
isProject: false
---
...
**Coupling footer width:** `4` trebuie să coincidă cu `1111`. Opțiuni:
- **`rest -4b`** — explicit, **Faza 2** (obligatoriu)
- **`rest -footer`** — nice-to-have Faza 2: suffix-ul `1111` din `out:` devine nume referibil; motorul calculează lățimea (4b) automat — dacă nu e fezabil, rămânem doar la `rest -4b`
...
- `rest -4b` explicit; **nice-to-have:** `rest -footer`
- LUT commit amânat (`pendingLutEntries`)
- Teste nesting 2–3 nivele, rollback izolat
...
| **Faza 2** | nesting, `rest -Nb`, LUT amânat; nice-to-have `rest -footer` |
| **Faza 3** | `parseView: tree` opțional + show + field access |
| **Faza 4** | documentație |
| **Faza 4+** | `parseTag`, schema union |
...
8. **`rest -footer`** — nice-to-have Faza 2; fallback `rest -4b` explicit.
