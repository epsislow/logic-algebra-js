...
todos:
  ...
  - id: logic-advanced-lists-2e
    content: "Faza 37 (2+e): liste avansate — dif-list, char-list builtins, lazy (subfaze F37a…F37c) — completed"
    status: completed
  - id: logic-dif-list-37a
    content: "F37a: dif-list L-H + append/2 dif-list + unificare — teste 4081+"
    status: completed
  - id: logic-char-list-37b
    content: "F37b: string_to_list/codes, atom_chars/codes — teste 4095+"
    status: completed
  ...
  - id: logic-lazy-streams-2j
    ...
    status: pending
  - id: logic-dcg-2i
    content: "Faza 38 (2+i): DCG — D297–D304 confirmed; F38a parse+expand+doc logic-dcg; F38b phrase+doc+teste — completed"
    status: completed
  - id: logic-dcg-38a
    content: "F38a: parse --> + expand DCG + doc logic-dcg partea 1 — teste 4141+"
    status: completed
  - id: logic-dcg-38b
    content: "F38b: phrase/2, phrase/3 engine + doc partea 2 + logic-builtins — teste 4149+"
    status: completed
  - id: logic-number-format-2k
    content: "Faza 39 (2+k): number/<format> la frontieră — u/s/q/fp codecs wire; extinde F25"
    status: completed
  - id: logic-number-format-39a
    content: "F39a: sintaxă number[/fmt] + default number/u<W> + signed s8…s64 — teste 4160+"
    status: completed
  - id: logic-number-format-39b
    content: "F39b: fixed-point q4p4 q8p8 qXpY — codec reutilizat — teste 4175+"
    status: completed
  - id: logic-number-format-39c
    content: "F39c: fp16 bf16 la frontieră — teste 4190+; doc comp-logic + logic-query-exec"
    status: completed
  - id: logic-float-2l
  ...
  - id: logic-float-40a
    ...
    status: completed
  - id: logic-float-40b
    content: "F40b: float/fp16 bf16 q* la frontieră; number/fp16|q* → eroare; migrare teste F39 — teste 4230+"
    status: completed
  - id: logic-float-40c
    content: "F40c: is/2 pe reale MVP (+ - * /, int→float) — teste 4250+; restul → 2+m"
    status: completed
  - id: logic-is-2m
    content: "Faza 42 (2+m): is/2 extins + random float — D332–D340✅ — teste 4386–4410"
    status: completed
  - id: logic-is-42a
    content: "F42a: operatori ** mod rem // în is/2 (int + float) — teste 4386+"
    status: completed
  - id: logic-is-42b
    content: "F42b: funcții unare abs sqrt floor ceiling round truncate în expr is — teste 4394+"
    status: completed
  - id: logic-is-42c
    content: "F42c: min/max binare în expr is — teste 4398+"
    status: completed
  - id: logic-is-42d
    content: "F42d: call(is/2) meta-call — teste 4400+"
    status: completed
  - id: logic-is-42e
...
> **Backlog post-MVP (**`1+a` **…** `1+v`**):** tabel complet — [Backlog post-MVP](#backlog-post-mvp). **Faze amânate (**`2+a` **…** `2+m`**):** — [Backlog faze amânate](#backlog-faze-amânate-2a--2m).
...
## Decizii Faza 25 — liste tipate pe wire + binding explicit (**2+c**, D182+) **(completed)**
...
| **Faza 37** liste avansate (**2+e**)           | **F37a✅ F37b✅ F37c✅** — D279–D296 **confirmed** | **(completed)** |
| **Faza 38** DCG **`phrase//`** (**2+i**)       | **F38a✅ F38b✅** — D297–D304 **confirmed** — [Faza 38](#faza-38--dcg-prolog-phrase-2i--completed) | **(completed)** |
| **Faza 39** **`number/<format>`** (**2+k**)    | **D305–D315✅** — extinde **F25**; [Faza 39](#faza-39--formate-numerice-la-frontieră-2k--ready-to-implement) | **(ready-to-implement)** |
| **Lazy streams / I/O** (**2+j**)               | **2+j** **(deferred)** — out of scope **F37c** (D296); promovat din F37c     |
...
2. **Faza 37** — **F37c** lazy + `between/3` (D293–D296 confirmed)
3. **Faza 30** — doc tutorial **mini-monopoly** (`doc/mini-monopoly-logic.md`) — doc-only, parallel
4. Apoi backlog **1+p**, **1+s**, **1+o**, …
5. Apoi faze amânate **2+a**, **2+b**, **2+f**, **2+i** (DCG → F38), **2+j** (lazy streams → post-F37c) · ~~**2+g**~~ → **F35** · ~~**2+e**~~ → **F37**
...
| ✅     | ~~**2+c**~~ | Liste tipate pe wire        | **Promovat → Faza 25** — D182–D199; mutation/program/query + codec listă; teste **3816+** | **F25**    | D182+, F22, F29 **(completed)** |
| ✅     | ~~**2+d**~~ | Builtins listă (core)       | `member/2`, `append/3`, `length/2`, `reverse/2`, `sort/2` | **F27**    | D137, D160–D168, F22 **(completed)** |
| ✅     | ~~**2+e**~~ | Liste avansate Prolog       | **Promovat → Faza 37** — **F37a✅ F37b✅ F37c✅**; vezi [Faza 37](#faza-37--liste-avansate-prolog-2e--completed) | **F37**    | D136, F22, F27, D279+ **(completed)** |
...
#### **2+c** ✅ → **F25** (D182–D199, extinde D59/D32/D140) **(completed)**

**Livrat** — secțiune completă: [Decizii Faza 25](#decizii-faza-25--liste-tipate-pe-wire--binding-explicit-2c-d182-completed).
...
**Livrat:** D182–D199 — codec listă pe wire; teste **3816–3840+** legacy+wave; doc EN migrate.

**Nu e în 2+c:** lazy listă pe wire/stream → **2+j**; `each` pe matrix rows → **F33** (livrat).
...
#### ~~**2+e**~~ ✅ → **Faza 37** (liste avansate) **(F37a✅ F37b✅ F37c✅ — completed)**

**Promovat 2026-08-25** — vezi [Faza 37](#faza-37--liste-avansate-prolog-2e--completed). **D279–D296 confirmed**. **F37a✅ F37b✅ F37c✅** livrate.
...
**Status:** **promovat → Faza 37** — **completed** (F37a…F37c).
...
**Ce este:** **Definite Clause Grammars** — reguli **`-->`** (D297), expand intern cu args listă ascunși, builtins **`phrase/2`**, **`phrase/3`**.

**Dependență:** **F37a✅** (dif-list `L-H`, `append/2`).

**Status:** **completed** — F38a parse+expand+doc p.1 · F38b `phrase/2,3`+doc+teste.

**Exemplu țintă LogTScript (D297-A):**
...
#### **2+j** ⏳ — Lazy streams / I/O (post-F37c)

**Amânat** — promovat în backlog la confirmarea **D296** (out of scope **F37c**).

**Sursă:** **[F37c](#f37c--lazy-4121-post-f37b--d289-c)** — F37c livrează `lazy_list/2` + `between/3` în memorie; **nu** include surse externe de stream.

**Ce este (draft):** lazy lists legate de **I/O** și **generatoare avansate** — inspirație SWI `library(lazy_lists)`:

| Livrabil draft | Descriere |
| -------------- | --------- |
| `lazy_read_lines/3` | stream → listă lazy de linii |
| `lazy_read_terms/3` | stream → listă lazy de termeni |
| Lazy pe **wire** / protocol | listă lazy din biți / pachete (legat **2+c** F25) |
| `lazy_findall/3` | soluții ca listă lazy (engines) |
| Integrare eager extinsă | `maplist`/`sort` pe lazy infinit — cu avertismente SWI-style |

**Dependență:** **F37c** (`lazy_list/2`, model D293) — obligatoriu înainte de **2+j**.

**Nu confunda cu:** **Faza 35** (builtins listă `numlist`, `member`, …) — alt scope; **2+j** vine din **F37c**, nu din F35.

**Status:** ⏳ backlog **2+j** — fără decizii proprii încă (streams/I/O; separat de **Faza 38** DCG **D297–D304**).
...
| **DCG** `phrase//`                                        | ✅ **F38** (ex-**2+i**)                      |
| **Liste pe wire**                                         | ✅ **F25** (ex-**2+c**)                      |
...
### F35e — Sublistă & combinatorică **(completed)**
...
### F35i — findall / bagof / setof **(completed)**
...
## Faza 37 — Liste avansate Prolog (**2+e**) **(completed)**

> **Promovat:** 2026-08-25 — **2+e** → fază normală **F37** cu **3 subfaze** (**F37a…F37c**).  
> **Decizii:** **D279–D296 confirmed** (D293–D296 user 2026-08-25).  
> **Livrat:** **F37a✅** dif-list · **F37b✅** char/codes · **F37c✅** lazy + `between/3`.  
...
> **Nu include:** DCG / `phrase//` (→ **[Faza 38](#faza-38--dcg-prolog-phrase-2i--completed)** / **2+i** ✅), lazy streams/I/O (→ **[2+j](#2j--lazy-streams--io-post-f37c)** / D296), unificare automată `"abc" = [97,98,99]` (D286-B), `append/3` schimbat.

### Problemă (stare azi post-F37c)

| Topic | Comportament azi |
| ----- | ---------------- |
| Liste cons `[H\|T]` | ✅ F22 — un capăt deschis (tail variabil) |
| `append/3` | ✅ F27 — concatenare clasică |
| Notație dif-list `L-H` | ✅ F37a — `dif_list(Front, Hole)`, `append/2` |
| `append/2` dif-list | ✅ F37a |
| String ↔ listă caractere/coduri | ✅ F37b — `string_to_*`, `atom_*` |
| Lazy list | ✅ F37c — `lazy_list/2`, `lazy_list_materialize/1` |
| `between/3` | ✅ F37c — range cu backtracking |

**Scop F37:** infrastructură Prolog avansată pentru liste — dif-list + builtins explicit char/codes + (opțional) lazy.

### Subfaze propuse

| Subfază | Livrabil | Status |
| ------- | -------- | ------ |
| **F37a** | Dif-list `Front-Hole`, unificare, `append/2`, `show` | ✅ **completed** |
| **F37b** | `string_to_list/2`, `string_to_codes/2`, `atom_chars/2`, `atom_codes/2` | ✅ **completed** |
| **F37c** | `lazy_list/2` (hibrid D293), `between/3`, integrare `member/2` | ✅ **completed** |
...
| **D290** | **DCG `phrase//`** | ✅ **B** — out of scope F37 → backlog **[2+i](#2i--faza-38-dcg-prolog)** / **[Faza 38](#faza-38--dcg-prolog-phrase-2i--ready-to-implement)** | **(confirmed)** |
| **D291** | **Head rezervat** | ✅ **A** — builtin call only (ca F27/F35) | **(confirmed)** |
| **D292** | **Teste & livrare** | ✅ **A** — **4081+** F37a; **4095+** F37b; **4121+** F37c; doc EN per subfază | **(confirmed)** |
| **D293** | **Lazy list model** | ✅ **D** — **hibrid B+A** (goal-template + regulă generator) | **(confirmed 2026-08-25)** |
| **D294** | **`between/3`** | ✅ **A** — în **F37c**, integer `[Low, High]` închis, SWI basic | **(confirmed 2026-08-25)** |
| **D295** | **Integrare lazy** | ✅ **A** — `member/2` da; `length/2` pe prefix; eager (`sort`, `maplist`, …) → fail clar | **(confirmed 2026-08-25)** |
| **D296** | **Out of scope F37c** | ✅ **A** — DCG → **F38**; lazy streams/I/O → backlog **[2+j](#2j--lazy-streams--io-post-f37c)** | **(confirmed 2026-08-25)** |
...
**La promovare F37c:** `lazy_list(+Goal, -List)` — model **D293** (hibrid B+A); plus **`between/3`** (D294).
...
**La noi azi:** `between/3` nu există; `numlist/3` (F35d) materializează tot. **F37c** adaugă ambele.

---

### D293 — Lazy list model **(confirmed: D — hibrid B+A)**

| Opțiune | Model | Pro | Contra |
| ------- | ----- | --- | ------ |
| **A — regulă generator** | `lazy_list(gen, Xs)` + `gen(Slice, Tail) <- …` | Extensibil, SWI-like callback | Necesită apel predicat cu 2 args |
| **B — goal-template** | `lazy_list(Xs, between(L, H, X))` | Ergonomic, exemplu plan direct | Whitelist pattern-uri goal |
| **C — doar `between/3`** | fără `lazy_list/2` general | Simplu | Nu e lazy list generală |
| **D — hibrid B+A ✓** | ambele forme de mai sus | Prolog-like + ergonomic pentru cazuri comune | Două căi de creare — documentare clară |

**Decizie:** **D** — **hibrid B+A** (user 2026-08-25, nedecis între A și B singure).

**Semantica țintă (ambele forme):**

1. **Goal-template (B):** `lazy_list(List, between(Low, High, X))` — sugar peste generator numeric intern.
2. **Regulă generator (A):** `lazy_list(Next, List)` unde `Next` e atom de predicat definit ca `next(Slice, Tail) <- Body` — apelat repetat ca `call(Next, Slice, Tail)`; `Slice\Tail` = următorul chunk (listă proprie; `Tail = []` la EOF).

**Term intern:** `kind: 'lazy_list'` cu referință generator + stare materializare (choice points la extindere).

**Opțional F37c:** `lazy_list_materialize/1` — listă închisă ground.

---

### D294 — `between/3` **(confirmed: A — în F37c)**

| Opțiune | Scope | Recomandare |
| ------- | ----- | ----------- |
| **A — SWI basic ✓** | `between(+Low, +High, ?Value)` — integers, `Low =< Value =< High`, backtracking crescător | **da** |
| **B — cu step** | `between/4` sau step opțional | post-F37c |
| **C — `inf` ca High** | interval infinit | post-F37c / **2+j** |

**Decizie:** **A** — în **F37c**, alături de `lazy_list/2`. `Low > High` → fail; non-integer → fail.

---

### D295 — Integrare lazy cu predicate existente **(confirmed: A)**

| Predicate | F37c |
| --------- | ---- |
| `member/2` | ✅ — materializează la mers, backtracking pe stream |
| `length/2` | ✅ — lungime **prefix materializat** sau fail dacă infinit și cerut closed |
| `append/3`, `sort/2`, `maplist/2`, `findall/3`, … | ❌ — **fail** cu mesaj clar pe term `lazy_list` (eager pe infinit = pericol SWI) |
| `show/N` | prefix materializat + marker dacă rest lazy (ex. `\|lazy\|`) |
| unificare `Lazy = [a,b,c]` | fail până la materializare completă (`lazy_list_materialize/1`) |

---

### D296 — Out of scope F37c **(confirmed: A)**

| Topic | Unde |
| ----- | ---- |
| DCG `rule//`, `-->`, `phrase/2`, `phrase/3` | **[Faza 38](#faza-38--dcg-prolog-phrase-2i--ready-to-implement)** / **2+i** (D290) |
| Lazy streams / file / wire / `lazy_findall` | Backlog **[2+j](#2j--lazy-streams--io-post-f37c)** — promovat din F37c |
| `maplist`/`sort` pe lazy infinit | **2+j** sau niciodată (documentat ca anti-pattern) |
| `between/4`, `inf` High | post-F37c |

**Decizie:** **A** — F37c = **lazy în memorie** + **`between/3`**; restul → backlog explicit **2+j** (nu F35 — F35 e builtins listă materializate).
...
**Decizie:** **B** — F37 livrează **dif-list + append/2**; DCG → backlog **[2+i](#2i--faza-38-dcg-prolog)** promovat ca **[Faza 38](#faza-38--dcg-prolog-phrase-2i--ready-to-implement)** (**D297–D304✅**).
...
#### F37c — lazy + `between/3` (4121+, post-F37b — D289-C, D293–D296)

| ID | Scenariu |
| -- | -------- |
| 4121 | `between(1, 3, X)` backtracking (unit) |
| 4122 | `between(3, 1, X)` fail; `between(1, 3, 2)` ground ok |
| 4123 | `lazy_list(Xs, between(1, 3, X))` + `member(Y, Xs)` — goal-template (legacy+wave) |
| 4124 | `lazy_list(gen, Xs)` + regulă `gen(Slice, Tail)` custom (legacy+wave) |
| 4125 | materializare parțială + `show` prefix |
| 4126 | backtracking pe stream (re-satisfy) |
| 4127 | `length(Lazy, N)` pe prefix materializat |
| 4128 | `sort(Lazy, _)` / `maplist(_, Lazy, _)` → fail clar |
| 4129 | regresie F37a/b (`append/3`, `member/2` pe liste normale) |
| 4130 | reserved head `lazy_list/2`, `between/3` |
...
| `[logic-engine.js](../v0_3_2/core/logic-engine.js)` | `lazy_list/2`, `between/3`, `member/2` pe lazy; (opț.) `lazy_list_materialize/1` |
...
**D279–D296 confirmed** (D293–D296 user 2026-08-25). **Următor pas:** implementare **F37c** (`lazy_list/2` + `between/3`).

| Confirmat | Decizie |
| --------- | ------- |
| D279–D285 | Dif-list model complet (A) — **F37a livrat** |
| D286–D288 | Char/codes explicit, bidirecțional (B + A + A) — **F37b livrat** |
| D289 | Lazy **F37c după F37b** — model implementare → **D293** |
| D290–D292 | Fără DCG în F37 → **2+i / F38**; heads rezervate; teste 4081+ / 4095+ / 4121+ |
| D293–D296 | Hibrid lazy B+A; `between/3`; integrare `member/2`; streams → **2+j** |

---

## Faza 38 — DCG Prolog (`phrase//`) **2+i** **(completed)**

> **Backlog:** **2+i** — promovat din **D290-B** (DCG out of scope F37).  
> **Dependență:** **F37a✅** dif-list (`L-H`, `append/2`) — îndeplinită.  
> **Extinde:** [Faza 22](#faza-22--liste-prolog-completed), [Faza 37](#faza-37--liste-avansate-prolog-2e--completed).  
> **Status:** **completed** — **F38a✅** parse `-->` + expand + doc p.1 · **F38b✅** `phrase/2`, `phrase/3` + doc p.2 + teste **4141+** / **4149+**.
...
| Sintaxă **`Head --> Body`** | ✅ F38a | parser DCG (distinct de `<-`) |
| Expand intern `//` (hidden list args) | ✅ F38a | la compile, după parse |
| `phrase/2`, `phrase/3` | ✅ F38b | builtins meta-parse |
| Terminale `[...]` în body DCG | ✅ F38a | consum/generare listă |

**Motiv D290-B:** DCG = parser + assembler + expand + engine — fază proprie, nu sub-fază F37.

### Scope (confirmat D297–D304)

| Livrabil | Subfază | Descriere |
| -------- | ------- | --------- |
| **Reguli `-->`** | F38a ✅ | `name(Args) --> Body` — args Prolog în head (D297-A) |
| **Body `{ }`** | F38a ✅ | Goals Prolog în `{}` obligatoriu (D299-B') |
| **Expand intern** | F38a ✅ | DCG → predicate cu 2 args listă ascunși (D298); heads expandate rezervate (D301) |
| **`logic-dcg.md` (partea 1)** | F38a ✅ | Pagină nouă — sintaxă, consum listă, exemple F38a (~8–10 `logts-play`) |
| **`phrase/2`** | F38b ✅ | `phrase(Goal, List)` — listă închisă (D300) |
| **`phrase/3`** | F38b ✅ | `phrase(Goal, List, Rest)` — dif-list `List-Rest` (D300) |
| **`logic-dcg.md` (partea 2)** | F38b ✅ | `phrase/2`, `phrase/3`, parse+generate, dif-list rest (~10–12 `logts-play`) |
| **Linkuri doc** | F38b ✅ | `inline-logic.md`, `logic-builtins.md`, `doc-viewer.js` |
| **Teste 4141+** | F38a+b ✅ | legacy+wave (D303) — split pe subfaze |

**Out of scope (D304):** DCG binar/wire, `{}` SWI full, `call_dcg/2` meta, `//2+` MVP.

### Documentație — `logic-dcg.md` **(pagină nouă, livrare incrementală)**

Model: ca **`logic-constraints.md`** — topic dedicat, nu secțiune în `logic-builtins.md`.

**Fișier:** `[doc/logic-dcg.md](../v0_3_2/doc/logic-dcg.md)` — creat la **F38a**, extins la **F38b**.

**Total țintă:** **~18–22** blocuri `logts-play` (nu doar 6) + verify `_verify_doc_examples.js logic-dcg`.

#### F38a — ce intră în pagină (partea 1)

| Secțiune | Conținut |
| -------- | -------- |
| Intro + quick reference | `-->` vs `<-`; tabel consum listă (`[...]` / NT / `{ }`) |
| Reguli DCG | head cu args; `//0` / `//1` echivalent (fără notație `//` la user) |
| Body B' | `{ between(0, 9, D) }`; `{ G1, G2 }`; eroare dacă goal fără `{}` |
| Terminale | `[D]`, `[=]`, `[]`, liste mixte |
| Non-terminale | `digits(Ds)` fără `{}` |
| Coexistență D301 | DCG `-->` + reguli `<-` în același inline; reserved head expandat |
| Out of scope (preview) | stub „phrase — F38b”; wire, `//2+`, `{}` SWI full |
| Linkuri | `append/2` dif-list, `between/3` — [logic-builtins.md](../v0_3_2/doc/logic-builtins.md) |

**Exemple `logts-play` F38a (~8–10)** — runnable unde se poate; restul erori de elaborare în teste suite:

| # | Scenariu | Ce demonstrează |
| - | -------- | --------------- |
| 1 | `digits` minimal `//1` | regulă `-->` + `{ between }` + NT recursiv |
| 2 | `digits([]) --> []` | echivalent `//0` |
| 3 | terminale `pair` cu `=` | NT + terminal atom |
| 4 | listă terminală multi-element `[1, 2, 3]` | consum secvențial |
| 5 | `{ G1, G2 }` în același `{}` | secvență goals fără consum |
| 6 | DCG + `<-` `is_digit` în același modul | D301 coexistență |
| 7 | grammar + query normală (non-DCG) | DCG nu strică reguli existente |
| 8 | eroare: goal fără `{}` | mesaj parse clar (test 414x; doc poate nota expected error) |
| 9 | eroare: user definește `digits/3` cu `<-` | reserved head expandat |
| 10 | (opțional) show/trace structură parse — dacă util | elaborare reușită |

**F38a — actualizări cross-doc:**

- `[inline-logic.md](../v0_3_2/doc/inline-logic.md)` — înlocuiește „DCG not supported” → link `logic-dcg.md`; paragraf scurt `-->` vs `<-`
- `doc-viewer.js` — intrare **Logic DCG** (partea 1 poate fi marcată „syntax”; partea 2 completează la F38b)

**Notă F38a:** fără `phrase/2,3`, exemplele runnable folosesc **elaborare + query-uri non-DCG** sau goals care nu necesită `phrase` (ex. coexistență, init OK). Secțiunea „Running grammars” = placeholder până la F38b.

#### F38b — ce se adaugă în pagină (partea 2)

| Secțiune | Conținut |
| -------- | -------- |
| `phrase/2` | parse listă închisă; generate |
| `phrase/3` | rest dif-list `List-Rest`; parse parțial |
| Bidirecțional | același grammar parse + generate (D300) |
| Reserved | `phrase/2`, `phrase/3` heads |
| Integrare | `comp [logic]` + `trigger`; legătură F37a `append/2` |
| Limitări MVP | D304 — fără wire, `call_dcg`, `//2+` |

**Exemple `logts-play` F38b (~10–12):**

| # | Scenariu |
| - | -------- |
| 1 | `phrase(digits, [1,2,3], [])` parse |
| 2 | `phrase(digits, Xs, [])` generate |
| 3 | `phrase/2` round-trip digits |
| 4 | `phrase/3` cu rest nevid |
| 5 | `phrase/3` + dif-list `Front-Hole` explicit |
| 6 | `pair` parse `3=7` |
| 7 | `pair` generate |
| 8 | grammar cu `{ member/2 }` în `{}` |
| 9 | grammar cu `{ append/2 }` dif-list helper |
| 10 | reserved head `phrase/2` error |
| 11 | `comp [logic]` pipeline complet (trigger) |
| 12 | (opțional) grammar nested NT + `between` |

**F38b — actualizări cross-doc:**

- `logic-builtins.md` — intrări `phrase/2`, `phrase/3` în quick reference + secțiune scurtă → link `logic-dcg.md`
- `inline-logic.md` — tabel features: DCG → link
- `doc-viewer.js` — descriere completă; regen `doc-data_generated.js`
- verify: `_verify_doc_examples.js logic-dcg` (toate blocurile F38a+F38b)

### Ordine recomandată

După **F37** complet (F37a+b+c ✅) — **F38a** parse `-->` + expand; **F38b** `phrase/2` + `phrase/3`.

### Rezumat decizii F38 **(D297–D304)**

| ID | Subiect | Status | Recomandare |
| -- | ------- | ------ | ----------- |
| **D297** | Sintaxă reguli DCG (`-->`) | ✅ **confirmed** 2026-08-25 | **A** — `-->` cu args în head |
| **D298** | Expand intern `//` (hidden list args) | ✅ **confirmed** 2026-08-25 | **A + B** — SWI 2 hidden args; MVP `//0` + `//1` |
| **D299** | Body DCG: `{}` și goals | ✅ **confirmed** 2026-08-25 | **B'** — `{}` **obligatoriu** pentru goals Prolog |
| **D300** | `phrase/2`, `phrase/3` | ✅ **confirmed** 2026-08-25 | **A + C** — bidirecțional; heads rezervate |
| **D301** | Coexistență DCG vs `<-` | ✅ **confirmed** 2026-08-25 | **B** — expand la compile; predicate expandat rezervat |
| **D302** | Subfaze F38 | ✅ **confirmed** 2026-08-25 | **C** — F38a expand; F38b phrase |
| **D303** | Teste (ID-uri) | ✅ **confirmed** 2026-08-25 | **A** — **4141+** legacy+wave |
| **D304** | Out of scope F38 | ✅ **confirmed** 2026-08-25 | **A** — confirmă draft (wire, `call_dcg`, `{}` full) |

### Decizii (detaliu)

#### D297 — Sintaxă reguli DCG **(confirmed: A — `-->` cu args în head)**

| Opțiune | Formă | Pro | Contra |
| ------- | ----- | --- | ------ |
| **A — `-->` + args în head ✓** | `digits([D\|Ds]) --> [D], between(0, 9, D), digits(Ds)` | Citibil; standard manuale Prolog; distinct clar de `<-` | Expand intern pentru list args |
| **B — `//N` în head** | `digits//1 --> [D], ...` | Compact SWI | Mai puțin intuitiv fără experiență DCG |
| **C — ambele** | A + B | Flexibil | Dublă sintaxă, mai mult de întreținut |

**Decizie:** **A** — user 2026-08-25: **îi place `-->`**; argumentele rămân în head ca la Prolog (`digits([D\|Ds])`, `digits([])`). **Nu** introducem `digits//1 -->` ca sintaxă de suprafață în MVP.

**Consecințe implementare:**
- **`<-`** — reguli logică normale (neschimbat)
- **`-->`** — **doar** reguli DCG; parse/assembler branch separat
- Head DCG: același format args ca la `<-` (`name(Arg1, …)`)
- Body DCG: terminale `[…]`, non-terminale, goals Prolog (`between/3`, etc.)
- `//N` poate exista doar **intern** la expand (D298), nu obligatoriu în textul scris de user
...
#### D298 — Expand intern DCG → predicate Prolog **(confirmed: A + B — SWI expand; MVP //0 + //1)**

Fiecare regulă `p(Args) --> Body` devine intern un predicate Prolog cu **2 argumente listă ascunse** (`List`, `Rest`), model SWI.

| Opțiune | Semnificație | Recomandare |
| ------- | ------------ | ----------- |
| **A — SWI 2 hidden args ✓** | `digits([D\|Ds]) --> …` → `digits([D\|Ds], List, Rest)` intern | **da** |
| **B — doar arity 0/1 MVP** | `//0` și `//1` echivalent; `//2+` amânat | pentru primul release |
| **C — fără expand** | interpret DCG direct în engine | mai lent, mai complex |

**Decizie:** **A + B** — user 2026-08-25.

**Exemple expand (intern, user nu scrie asta):**

| Regulă DCG (D297) | Predicate intern (după expand) |
| ----------------- | ------------------------------ |
| `digits([]) --> []` | `digits([], L, L)` — succes când consumă nimic |
| `digits([D\|Ds]) --> [D], …` | `digits([D\|Ds], L0, L)` cu legături listă din body |

---

#### D299 — Body DCG: terminale, non-terminale, `{}` **(confirmed: B' — `{}` obligatoriu pentru goals Prolog)**

| Fragment în body | Consumă listă? | Sintaxă |
| ---------------- | -------------- | ------- |
| `[a, b]`, `[D]`, `[]` | **da** — terminal | fără `{}` |
| `digits(Ds)` | **da** — non-terminal DCG | fără `{}` (trebuie regulă `-->`) |
| `{ between(0, 9, D) }` | **nu** — goal Prolog | **`{}` obligatoriu** |
| `{ G1, G2 }` | **nu** — secvență goals | **`{}` obligatoriu** |

| Opțiune | `{}` în body | Status |
| ------- | ------------ | ------ |
| **A — fără `{}`** | Goals direct: `between(0, 9, D)` | respins |
| **B — `{}` opțional** | sugar echivalent cu goal fără acolade | respins (prea ambiguu) |
| **B' — `{}` obligatoriu ✓** | orice goal Prolog **trebuie** în `{}` | **confirmed** |
| **C — `{}` SWI complet** | freeze, `call/1`, semantica extra | **out of scope** (D304) |

**Decizie:** **B'** — user 2026-08-25: claritate „ce consumă / ce nu”; aliniat manuale Prolog DCG.

**Reguli body DCG (B'):**

1. **`[...]`** — terminal (consumă/generează din listă de intrare)
2. **`name(Args)`** fără `{}` — **doar** apel non-terminal DCG (există regulă `-->` cu același nume/arity vizibil)
3. **`{ Goal }`** sau **`{ G1, G2, … }`** — goal(uri) Prolog care **nu consumă** listă (builtins, reguli `<-`, unificări, `is`, etc.)
4. Goal Prolog **fără** `{}` în body DCG → **eroare parse** (mesaj: „DCG goal must be braced: `{ … }`”)
...
**Tokenizare `{` `}` — fără conflict:**

- `logic-assembler.js` (`logicTokenize`) **nu** are încă token `{` / `}` — sunt **libere** de adăugat
- `logic { + / - }` din **exec block** `comp [logic]` e parsat de **alt** parser (nivel LogTScript), nu de body-ul `inline [logic]`
- Tokenii `LBRACE` / `RBRACE` se activează **doar** în parserul body DCG (după `-->`), nu în reguli `<-` sau query-uri

**Alternative sintactice** (doar dacă apare problemă la implementare — **preferat: `{` `}`**):

| Token | Exemplu | Pro | Contra |
| ----- | ------- | --- | ------ |
| **`{` `}` ✓** | `{ between(0, 9, D) }` | standard SWI/Prolog DCG | trebuie adăugați 2 tokeni (simplu) |
| `{{` `}}` | `{{ between(0, 9, D) }}` | zero ambiguitate vizuală | neobișnuit |
| `do` … `end` | `do between(0, 9, D) end` | fără caractere speciale | verbos, non-Prolog |
| `\|` … `\|` | `\| between(0, 9, D) \|` | distinct de `\|` listă | confuz cu pipe cons |

**Recomandare implementare:** `{` `}` (D299-B'); alternativele rămân fallback documentat, nu schimbă decizia semantică B'.

---

#### D300 — `phrase/2` și `phrase/3` **(confirmed: A + C — bidirecțional; heads rezervate)**

| Builtin | Semantica propusă |
| ------- | ----------------- |
| **`phrase(Goal, List)`** | Echivalent `phrase(Goal, List, [])` — listă închisă consumată integral |
| **`phrase(Goal, List, Rest)`** | `List-Rest` dif-list; parse parțial sau generate cu rest |

| Opțiune | Comportament | Recomandare |
| ------- | ------------ | ----------- |
| **A — bidirecțional ✓** | Parse **și** generate (SWI) | **da** |
| **B — doar parse** | `phrase` consumă listă | prea limitat |
| **C — heads rezervate** | user nu poate defini `phrase/2` ca regulă | **da** (ca F27) |

**Decizie:** **A + C** — user 2026-08-25. `phrase/2` și `phrase/3` reserved heads (pattern F27); parse **și** generate în MVP.

---

#### D301 — Coexistență DCG (`-->`) vs reguli normale (`<-`) **(confirmed: B — expand la compile; predicate expandat rezervat)**

| Opțiune | Comportament | Recomandare |
| ------- | ------------ | ----------- |
| **A — separate strict** | Același nume nu poate avea și `<-` și `-->` | clar, dar restrictiv |
| **B — expand la compile ✓** | `digits([D\|Ds]) --> …` → `digits/3` intern; user **nu** definește manual `digits/3` cu `<-` | **SWI-like** |
| **C — ambele permise** | Confuzie la apel | nu |

**Decizie:** **B** — user 2026-08-25. DCG expandat la compile; predicate-ul expandat (ex. `digits/3` cu 2 list args) e **rezervat** — user nu poate defini același nume+arity cu `<-`.

---

#### D302 — Subfaze F38 **(confirmed: C — F38a expand; F38b phrase)**

| Opțiune | Împărțire | Estimare |
| ------- | --------- | -------- |
| **A — 3 subfaze** | F38a parse+expand; F38b `phrase/2`; F38c `phrase/3`+dif-list | incremental |
| **B — o fază** | tot dintr-o dată | ~1–2 săpt. |
| **C — 2 subfaze ✓** | **F38a** parse `-->` + expand; **F38b** `phrase/2` + `phrase/3` | echilibrat |

**Decizie:** **C** — user 2026-08-25.

| Subfază | Livrabil cod | Livrabil doc |
| ------- | ------------ | ------------ |
| **F38a** | Token `-->` + `LBRACE`/`RBRACE`; parse DCG clauses; expand intern; validate heads rezervate expandate; teste **4141–4144**, **4155**, **4157** (parse/expand/reserved) | **`logic-dcg.md` partea 1** (~8–10 `logts-play`); `inline-logic.md` stub link; `doc-viewer` intrare |
| **F38b** | `phrase/2`, `phrase/3` engine; reserved heads; teste **4145+**, **4151+** (phrase, generate, dif-list) | **`logic-dcg.md` partea 2** (~10–12 `logts-play`); `logic-builtins.md` `phrase/*`; verify `logic-dcg` complet |

---

#### D303 — Teste **(confirmed: A — 4141+ legacy+wave; split F38a / F38b)**

| ID | Subfază | Scenariu |
| -- | ------- | -------- |
| 4141 | F38a | parse `-->` / DCG clause struct |
| 4142 | F38a | expand `digits//0` echivalent (`digits([])`) |
| 4143 | F38a | expand `digits//1` echivalent |
| 4144 | F38a | goal fără `{}` → parse error |
| 4145 | F38b | `phrase/2` parse listă închisă (legacy+wave) |
| 4147 | F38b | `phrase/3` cu rest dif-list (legacy+wave) |
| 4151 | F38b | generate round-trip (legacy+wave) |
| 4155 | F38a | regresie F37a (`append/2`, dif-list) |
| 4157 | F38b | reserved head `phrase/2`, `phrase/3` |
| 4159 | F38b | reserved head predicate expandat (ex. `digits/3`) |

**Decizie:** **A** — user 2026-08-25. Perechi **legacy+wave** pentru integrare; unit tests pentru parse/expand.

---

#### D304 — Out of scope F38 **(confirmed: A — draft out of scope)**

| Topic | Unde |
| ----- | ---- |
| DCG pe wire / protocol binary | backlog viitor (nu F38) |
| `call_dcg/2`, `phrase/4` meta | **F38b** sau amânat |
| `{}` semantica SWI completă | amânat |
| Module / `public` DCG separate | amânat |
| `//2+` în MVP | post-F38 (dacă D298-B) |

**Decizie:** **A** — user 2026-08-25. Confirmă draft; la fel ca D296 pentru F37c.

---

### Decizii F38 — status **(toate confirmed 2026-08-25)**

| ID | Decizie |
| -- | ------- |
| D297 | **`-->`** cu args în head (A) |
| D298 | Expand SWI +2 list args; MVP echivalent **//0** și **//1** (A + B) |
| D299 | **`{}` obligatoriu** pentru goals Prolog în body DCG (B') |
| D300 | **`phrase/2`, `phrase/3`** bidirecțional + heads rezervate (A + C) |
| D301 | Expand la compile; predicate expandat **rezervat** (B) |
| D302 | **F38a** parse+expand+**doc p.1** → **F38b** `phrase/2`+`phrase/3`+**doc p.2** (C) |
| D303 | Teste **4141+** legacy+wave; split F38a/F38b (A) |
| D304 | Out of scope: wire DCG, `call_dcg`, `{}` SWI full, `//2+` MVP (A) |

**Următor pas implementare:** **[Faza 39](#faza-39--formate-numerice-la-frontieră-2k--ready-to-implement)** (`number/<format>`) · apoi **2+j** / **2+a** / **2+b** / **2+f**.

### Legături

- Tabel backlog: [2+i](#2i--faza-38-dcg-prolog) · [2+k](#2k--formate-numerice-la-frontieră-faza-39)
- D290 confirmed: DCG → F38, nu F37
- Formate wire: reutilizează tagged builtins (`q4p4`, `fp16`, `sX`) — [builtin-tagged-index.md](../v0_3_2/doc/builtin-tagged-index.md)

---

## Faza 39 — Formate numerice la frontieră **`number/<format>`** **2+k** **(completed)**

> **Backlog:** **2+k** — promovat 2026-08-25; **livrat F39a→c** (2026-08-25).  
> **Extinde:** [Faza 25](#decizii-faza-25--liste-tipate-pe-wire--binding-explicit-2c-d182-completed) (D182–D199 — `text|number|bool [list]`), [Faza 33](#faza-33--mutation-each-expansion-completed) (`each`), [D14](#d14--tipuri-la-frontieră-completed-a--number--bool--text-mvp) (tip la pin).  
> **Notă F40:** formatele fracționale/IEEE de la frontieră (`q*`, `fp16`, `bf16`) trec la **`float/<format>`** în F40; `number/q*` și `number/fp16` devin **eroare** — vezi [Faza 40](#faza-40--float-prolog--frontieră-floatformat-2l--ready-to-implement).  
> **Status:** **completed** — **D305–D315✅**; teste **4160–4203**.

### Problemă (stare azi post-F38)

| Topic | Comportament azi |
| ----- | ---------------- |
| Binding `number` / `number list` | ✅ F25 — trei suprafețe (mutation, program block, query) |
| Decode scalar `number` | **Unsigned** — `parseInt(bits, 2)` pe **toți biții wire-ului** |
| Lățime implicită | **Wire width** (8wire→8b, 32wire→32b); pin comp default **64** înainte de assign |
| Signed (`s32`, …) | ❌ — nu există |
| Fixed (`q4p4`, `qXpY`) | ❌ în logic; există pe **tagged builtins** |
| Float wire (`fp16`, `bf16`) | ❌ în logic |
| KB Prolog | `{ kind: 'number', value: int }` — **fără float** în termeni |
| `number list` packed scalar | **16 biți/celulă** fix (nu urmează element width wire) |

**Motiv fază:** LogTScript are deja formate numerice pe wire (`q4p4`, `fp16`, `s8`, …); logic folosește doar **unsigned pe lățimea wire-ului**. Jocuri / senzori / fixed-point necesită **codec explicit** la frontieră, aliniat cu restul limbajului.

### Scope (D305–D315)

| Livrabil | Subfază | Descriere |
| -------- | ------- | --------- |
| **Sintaxă `number/<format>`** | F39a | Extinde gramatica F25: `number [/ format] [list] [each] wire` |
| **Default `number`** | F39a | Echivalent **`number/u<W>`** — `W` = lățime element wire/pin (sau 64 pin comp neasignat) |
| **Unsigned explicit** | F39a | `u8` `u16` `u32` `u64` `uX` (parametric, ca tagged builtins) |
| **Signed** | F39a | `s8` `s16` `s32` `s64` `sX` — two's complement decode/encode |
| **Fixed-point** | F39b | `q4p4` `q8p8` `qXpY` — reutilizează codec interpreter |
| **IEEE half** | F39c | `fp16` `bf16` — decode/encode la frontieră |
| **Liste + each** | F39a–c | `number/s32 list`, `number/q4p4 list each M` — același format per celulă |
| **Validare lățime format** | F39a | `formatWidth` trebuie să coincidă cu lățimea wire/pin (element width pe vector) — altfel **eroare elaborare** (D315) |
| **Backward compat** | F39a | `number`, `number list`, `X=number` — comportament identic cu azi (= `u<W>`) |
| **Doc** | F39a–c | `comp-logic.md`, `logic-query-exec.md`, `logic-value-types.md`, `logic-runtime.md` |
| **Teste 4160+** | F39a–c | legacy+wave |

**Out of scope F39 (D314):** literale float în Prolog (`1.5`); `float/1`; `is/2` pe reale; `atom_number/2`; format `df`; infer format din lățime fără `/` (păstrăm explicit ca la D196); **packed scalar `number list`** rămâne **16 biți/celulă** fix (vezi D314).

### Sintaxă canonică (țintă)

Extinde vocabularul F25 — **un singur keyword scalar numeric: `number`** (fără `float` la suprafață).

```text
bindType   := number [/ numberFormat] | text | bool
numberFormat := u8 | u16 | u32 | u64 | uX
              | s8 | s16 | s32 | s64 | sX
              | q4p4 | q8p8 | q<N>p<M>    ; parametric qXpY
              | fp16 | bf16
listMod    := list [each]
```
...
| Suprafață | Exemplu |
| --------- | ------- |
| **Program block** | `Score is number/q4p4 scorePin` · `Delta is number/s32 deltaWire` |
| **Query input** | `T = number/fp16 sensorIn` |
| **Query output hint** | `T = number/s32` (fără wire) |
| **Mutation** | `+ reading(box, number/fp16 list sensors)` |
| **Each (F33)** | `number/q4p4 list each matrixWire` |

**Default:**

| Scrii | Semnificație internă |
| ----- | -------------------- |
| `number` pe `16wire` | `number/u16` — unsigned, 16 biți |
| `number` pe `32wire` | `number/u32` |
| `number list` pe `16wire[4]` | `number/u16 list` — 16 biți per element |
| `X is number myX` (pin comp, neasignat) | `number/u64` (ca azi) |

### Valoare în KB după decode (D307)

| Format frontieră | Valoare logică în KB (MVP F39) |
| ---------------- | ------------------------------ |
| `u*` / `s*` | `{ kind: 'number', value: int }` — integer Prolog |
| `q4p4`, `qXpY` | `{ kind: 'number', value: int }` — **raw fixed-point** (biți întregi); conversie „umană” în reguli dacă e nevoie |
| `fp16`, `bf16` | `{ kind: 'number', value: int }` — **raw IEEE bits** ca integer (MVP); documentat; fără kind `float` în KB |

**Notă:** formatul (`numberFormat`) se păstrează în **metadata binding** (pin, query hint, mutation wire ref) pentru encode înapoi; **nu** în termenul Prolog din KB.

### Subfaze

| Subfază | Livrabil | Teste (draft) |
| ------- | -------- | ------------- |
| **F39a** | Parse `number/<fmt>`; `logicPinToInputValue` / encode cu format; `u<W>` default; signed `s*`; regresie `number` simplu | **4160+** |
| **F39b** | `q4p4`, `q8p8`, `qXpY` — bridge la codec numeric existent | **4175+** |
| **F39c** | `fp16`, `bf16`; liste + each cu format; doc complet | **4190+** |

### Decizii **(D305–D315✅ confirmed 2026-08-25)**

#### D305 — Keyword unic `number` **(confirmed: A — fără `float` la frontieră)**

| Opțiune | Formă | Pro | Contra |
| ------- | ----- | --- | -------- |
| **A — `number/<format>` ✓** | `number/fp16`, `number/s32`, `number/q4p4` | Extinde F25; un vocabular; `fp16` e format, nu kind | `number/fp16` „sună” ca int |
| **B — `float/<format>` + `number/<format>`** | split la binding | Clar conceptual pentru IEEE | Dublă gramatică; `float/q4p4` ciudat |

**Decizie:** **A** — user 2026-08-25.

#### D306 — Default `number` = `number/u<W>` **(confirmed: A)**

`W` = lățimea în biți a wire-ului sursă (sau element width pentru vector); pin comp `number` neasignat = **64** (neschimbat).

#### D307 — Stocare KB **(confirmed: A — int în termen; format la frontieră)**

Format doar în AST binding / pin metadata; KB rămâne `kind: 'number'`.

#### D308 — Signed MVP **(confirmed: A — s8…s64 + sX)**

Two's complement simetric cu `logicNumberToBits` la encode.

#### D309 — Fixed-point **(confirmed: A — reutilizează tagged codec)**

Nu reimplementa qXpY în logic-engine — apel shared din interpreter / modul numeric.

#### D310 — fp16/bf16 **(confirmed: A — raw bits în KB la MVP)**

Evită float în unificare/`is/2`; encode/decode round-trip pe wire.

#### D311 — Suprafețe **(confirmed: A — aceleași trei ca F25)**

Mutation, program block, query input/output — plus `each` (F33).

#### D312 — Backward compat **(confirmed: A)**

`number` fără `/` = comportament actual (unsigned pe lățime wire).

#### D313 — Teste **(confirmed: A — 4160+ legacy+wave)**

Split F39a/b/c ca F38.

#### D314 — Out of scope F39 **(confirmed: A)**
...
**D314 — packed `number list` (explicație):**

La **`number list`** există **două moduri** (F25):

| Wire | Lățime per element azi |
| ---- | ---------------------- |
| **Vector** `16wire[4]` | **element width** wire-ului (16 pe `16wire`) |
| **Scalar packed** (wire lat **fără** `[N]`, ex. `32wire packedIn`) | **16 biți fix** per slot — hardcodat în engine (`logicListPackedElementWidth`) |

**Ce NU face F39:** nu schimbă regula packed scalar — rămâne **împărțire la 16 biți/celulă**, indiferent dacă scrii `number/s32 list` sau `number/u8 list` pe un wire packed non-vector.

**Exemplu azi:**
```logts
Vals = number list packedIn   ; packedIn = 32wire (fără [N])
; → 2 elemente × 16 biți, NU 4 × 8 biți
```

**Ce FACE F39 pentru liste:** pe **vector** (`8wire[N]`, `16wire[N]`, …), `number/s32 list` aplică formatul **per celulă** (element width + codec int). Formate fracționale/IEEE pe listă trec la **`float/<fmt> list`** în **F40**; packed scalar rămâne 16 fix până la **F40e** (D319).

**Motiv amânare:** comportament vechi F25 + teste existente; schimbarea ar putea rupe scripturi care se bazează pe 16 bit/slot packed.

#### D315 — Mismatch format vs lățime wire **(confirmed: A — eroare elaborare)**

| Opțiune | Comportament | Pro | Contra |
| ------- | ------------ | --- | ------ |
| **A — eroare elaborare ✓** | `number/u8` pe `32wire` → **fail** la elaborare | Aliniat cu LogTScript: LHS/RHS **bit-width mismatch** → error (`Bit-width mismatch: …`) | User trebuie să aleagă format corect sau wire potrivit |
| **B — low W biți** | decode/encode pe low W biți | Permisiv | Ascunde bug-uri de design; inconsistent cu restul limbajului |
| **C — pad/trunchiere** | reguli implicite | Flexibil | Surprize la round-trip |

**Decizie:** **A** — user 2026-08-25.

**Regulă:** la bind elaboration, `formatWidth(format)` trebuie să fie **egal** cu lățimea wire-ului țintă (sau **element width** pe vector `Wwire[N]`). Dacă `formatWidth ≠ wireWidth` → eroare elaborare (mesaj în familia `Bit-width mismatch` / `number format width … does not match wire …`).

**Exemple:**

| Binding | Wire | Rezultat |
| ------- | ---- | -------- |
| `number/u8` | `8wire` | ✅ OK |
| `number/u8` | `32wire` | ❌ elaboration error |
| `number` (implicit `u32`) | `32wire` | ✅ OK |
| `number/s32 list` | `32wire[4]` | ✅ OK (32 = element width) |
| `number/u8 list` | `32wire[4]` | ❌ elaboration error |

**Teste F39a:** include caz negativ `number/u8` pe `32wire` în **4160+**.

### Exemplu țintă

```logts
inline [logic] .sensors:
...
comp [logic] .sensorLogic:
    on: 1
    .sensors {
        T is number/q4p4 tempPin
    }
:

16wire tempIn = ...   ; q4p4 raw pe wire
...
```logts
; query inline — signed
1wire ok = .world:query(
    { between(-5, 5, X) },
    X = number/s32 deltaIn
)
```
...
| Layer | Schimbare |
| ----- | --------- |
| `logic-assembler.js` | `parseMutationTerm`, `parseLogicProgramBlock`, query bindings — `number` + optional `/format` |
| `parser.js` | `.world:query` bindings — același pattern |
| `logic-engine.js` | `logicPinToInputValue(bits, bindType, numberFormat)`; `logicTermToWireValue(..., format)`; list codec per-format element width |
| `components/logic.js` | Pin metadata `numberFormat`; pin width rules cu format |
| `interpreter.js` | Pass format la decode/encode; bridge `qXpY`/`fp16` |
| **Shared** | Extrage sau apelează funcții existente tagged builtins / NFORMAT |
| **Teste** | 4160+; migrate câteva F25 să folosească explicit `number/u8` |
| **Doc EN** | Tabele format; default `u<W>`; exemple `logts-play` |

**Estimare:** medie — parse pe 3 suprafețe + codec matrix; F39a mică; F39b/c depind de reuse numeric core.

### Rezumat decizii F39 **(D305–D315✅)**

| ID | Subiect | Decizie |
| -- | ------- | ------- |
| **D305** | Keyword frontieră | **A** — doar `number/<format>` |
| **D306** | Default | **A** — `number` = `number/u<W>` |
| **D307** | KB | **A** — int în termen; format la binding |
| **D308** | Signed | **A** — `s8`…`s64`, `sX` |
| **D309** | Fixed | **A** — reuse codec `qXpY` |
| **D310** | fp16/bf16 | **A** — raw bits MVP |
| **D311** | Suprafețe | **A** — F25 + each |
| **D312** | Compat | **A** — `number` simplu neschimbat |
| **D313** | Teste | **A** — **4160+** legacy+wave |
| **D314** | Out of scope | **A** — float Prolog, `atom_number`, `df`; **packed list 16 fix neschimbat** |
| **D315** | Format vs wire width | **A** — **eroare elaborare** dacă `formatWidth ≠ wireWidth` |

**Următor pas implementare:** **F39a** — parse `number/<format>` + `u<W>` default + signed decode/encode + teste **4160+**.

#### **2+k** ⏳ → **Faza 39** (formate numerice la frontieră)

**Promovat 2026-08-25** — vezi [Faza 39](#faza-39--formate-numerice-la-frontieră-2k--ready-to-implement).

**Ce este:** extensie **F25** — același `text` / `number` / `bool`, dar `number` poate specifica **codec wire** (`/u32`, `/s32`, `/q4p4`, `/fp16`, …).

**Nu este:** float în engine Prolog; `is/2` pe reale; înlocuirea tagged builtins din expresii wire.
...
#### **2+m** ✅ → **F42** (D332–D340 confirmed; livrat 2026-08-26)

**Promovat:** 2026-08-25 — restul aritmeticii float scoasă din scope **F40c** (D318). **Renumerotat F42** post-**F41✅**.

**Livrat** — secțiune: [Faza 42 — `is/2` extins + random float](#faza-42--is2-extins--random-float-2m--completed). Teste **4386–4410**; **3542/3542**; doc **logic-builtins** 76/76.
...
| Topic | Comportament azi (F39) |
| ----- | ---------------------- |
| `number`, `number/u*`, `number/s*` | ✅ int la frontieră + în KB |
| `number/q4p4`, `number/fp16`, `number/bf16` | ❌ eroare — folosește `float/q*`, `float/fp16`, `float/bf16` |
| `float/<fmt>` la frontieră | ✅ F40b — valoare umană în KB (`kind: float`) |
| `float/X`, `float list` packed f32 | ✅ F40b |
| Literale Prolog `1.5` | ✅ F40a — kind `float`, `1.5`, `-0.25`, `.5` |
...
| Livrabil | Subfază | Descriere |
| -------- | ------- | --------- |
| **Kind `float` + literale** | F40a | `1.5`, `-0.25` în facts/reguli; `{ kind: 'float', value: number }` (IEEE double); `float/1` |
| **Split frontieră `number` vs `float`** | F40b | `float/q4p4`, `float/fp16`, `float/bf16`, `float/qXpY` — valoare umană în KB; **`number/fp16`**, **`number/q*`** → **eroare elaborare** |
| **`float list` / `each`** | F40b | `float/fp16 list`, `float/q4p4 list each` — același pattern F25/F33 |
| **`is/2` reale** | F40c | `X is 1.5 + 2.0`, funcții minime pe float |
| **`atom_number/2`** | F40d | atom ↔ int/float parse/format |
| **Packed list + tag-uri `f32`/`f64`** | F40e | `float list` packed (default 32-bit); tag-uri `f32`/`f64` la frontieră pentru D319 |

**Migrare F39 → F40:** teste/doc **4175–4203** (`number/q*`, `number/fp16`) → **`float/q*`**, **`float/fp16`**; facts cu raw int (`15360`) → literale float (`1.0`) unde e cazul.

### Sintaxă canonică (țintă)

Extinde vocabularul F25 — **două keywords scalare numerice la frontieră:** `number` (int) și `float`.

```text
bindType   := number [/ intFormat] | float [/ floatFormat] | text | bool
intFormat  := u8 | u16 | u32 | u64 | uX | s8 | s16 | s32 | s64 | sX
floatFormat := q4p4 | q8p8 | q<N>p<M> | fp16 | bf16 | f32 | f64 | X
listMod    := list [each]
```

| Suprafață | Exemplu |
| --------- | ------- |
| **Query input** | `T = float/fp16 sensorIn` → `1.0` în KB |
| **Program block** | `Temp is float/q4p4 tempPin` |
| **Mutation** | `+ reading(box, float/fp16 list sensors)` |
| **Fact** | `threshold(1.5).` · `expect(-2.75)` |
| **Respins** | `T = number/fp16 sensorIn` → **eroare** (`use float/fp16`) |

**`number`** fără `/` = neschimbat (`number/u<W>`). **`float`** fără `/` = **`float/X`** (parametric — lățime element → `fp16`/`f32`/`f64`; vezi [D319](#d319--default-float-fără--liste-packed-confirmed-a--floatx--packed-32-bit)). **`float`** pe pin comp neasignat = **64** biți (ca `number`).

### Valoare în KB după decode (post-F40)

| Frontieră | Valoare logică în KB |
| --------- | -------------------- |
| `number`, `number/u*`, `number/s*` | `{ kind: 'number', value: int }` |
| `float/q4p4`, `float/qXpY` | `{ kind: 'float', value: IEEE }` — ex. `1.5` |
| `float/fp16`, `float/bf16` | `{ kind: 'float', value: IEEE }` — ex. `1.0` |
| Literale `1.5` în sursă | `{ kind: 'float', value: IEEE }` |
...
#### D316 — Kind `float` în KB **(confirmed: A — kind nou)**

| Opțiune | Formă | Pro | Contra |
| ------- | ----- | --- | -------- |
| **A — kind `float` ✓** | `float/1`, literale `1.5`, frontieră `float/<fmt>` | Clar int vs float; `number/1` rămâne integer | Două keywords la frontieră |
| B — `number` cu fracții | un singur kind | Mai puțin vocabular | Rupe F26, `number/1`, unificare |

**Decizie:** **A** — user 2026-08-25. Implicație: formate fracționale/IEEE la frontieră devin **`float/q4p4`**, **`float/fp16`**, nu `number/…`.

#### D317 — Frontieră IEEE/fixed **(confirmed: B — `float/<fmt>`; `number/fp16|q*` eroare)**

| Opțiune | `number/fp16` | `float/fp16` | KB |
| ------- | ------------- | ------------ | --- |
| **A** | raw int (F39) | — | int |
| **B — `float/<fmt>` ✓** | **eroare elaborare** | decode → valoare umană | `kind: float` |
| C | raw int | decode float | ambele coexistă |

**Decizie:** **B** — user 2026-08-25. Mesaj tip: `number format 'fp16' is not an integer format (use float/fp16)`.

#### D318 — `is/2` pe reale **(confirmed: A — MVP în F40c; extins în 2+m)**

**F40c (MVP):** extinde F26 cu float în RHS:

| În scope F40c | În afara scope → **[2+m](#2m--is2-float-extins-post-f40c)** |
| ------------- | ------------------------------------------------------------- |
| `+` `-` `*` `/` pe float | `**` (putere) |
...
**Legat de:** [Faza 26](#faza-26--is2-evaluare-aritmetică-prolog--completed) (out of scope float la livrare — acum F40c); backlog **[2+m](#2m--is2-float-extins-post-f40c)**.

#### D319 — Default `float` fără `/` + liste packed **(confirmed: A — `float/X` + packed 32-bit)**

**Problema:** utilizatorul nu vrea `float/fp16` obligatoriu peste tot — vrea `float sensorIn` și `float list vals` „normale”.

**Decizie:** același pattern ca **`number` → `number/u<W>`** și **`uX`/`sX`** — keyword **`float` fără slash = `float/X`** (parametric după **lățimea elementului** wire/pin).

| Lățime element (W) | Format implicit (`float/X`) | Valoare în KB |
| ------------------ | --------------------------- | ------------- |
| **16** | `fp16` | IEEE half ca float (ex. `1.0`) |
| **32** | `f32` (IEEE single — tag nou la frontieră logic) | ex. `1.5` |
| **64** | `f64` (IEEE double — tag nou) | ex. `1.5` |
| **8**, **12**, alte W fixe q | **eroare elaborare** | — folosește explicit `float/q4p4`, `float/q8p8`, … |

**Scalar / pin / query binding:**

```text
T = float sensorIn          ; pe 32wire  → același ca  T = float/f32 sensorIn
Temp is float tempPin       ; pin ia formatul din lățimea wire la assign
```

**Vector** `Wwire[N]` — `float list vecIn` = **`float/X list`**: N celule, fiecare cu codec din `W`.

**Packed scalar** (wire **fără** `[N]` — total bits ÷ slot size):

| Formă | Slot size default | Note |
| ----- | ----------------- | ---- |
| **`float list`** (fără `/`) | **32 biți/celulă** (`f32`) | Contrast: `number list` packed rămâne **16 fix** (D314) |
| **`float/fp16 list`** | 16 biți/celulă | explicit, dens |
| **`float/f64 list`** | 64 biți/celulă | explicit, precizie dublă |

Exemplu: `128wire packed` + `Vals=float list packedIn` → **4** float-uri f32 în KB.

**Explicit rămâne disponibil:** `float/fp16`, `float/q4p4`, … când vrei format fix indiferent de lățime (cu validare D315 width match).

#### D320 — (închis — absorbit în D319)

Default pin comp `float` neasignat: **64 biți** (ca `number` azi), la assign → `float/X` din lățimea wire; plafon 64. Detalii pin în doc `comp-logic.md` la implementare F40b.

#### D321 — Tag-uri frontieră IEEE **`f32`** / **`f64`** **(confirmed: A)**

Tag-uri noi la frontieră logic (nu în `FORMAT_TAG_NAMES` builtins wire azi): **`f32`** = IEEE single 32-bit, **`f64`** = IEEE double 64-bit. Folosite de **`float/X`** (D319) pe `32wire` / `64wire`.

#### D322 — Unificare int vs float **(confirmed: A — fail)**

`1 = 1.0`, `1 =:= 1.0`, `number(1.0)` — **fail** / fals. Tipuri distincte în KB.

#### D323 — Gramatică literale float **(confirmed: A)**

`1.5`, `-0.25`, `.5` — da. Notație științifică `1e10` — **nu** la F40a (poate în **2+m**).

#### D324 — NaN / Infinity **(confirmed: A)**

Stocate ca JS `Number` în KB; decode wire în F40b; `is/2` pe NaN/Inf — comportament F40c (MVP: `/0` → fail).

#### D325 — Ordine tag-uri în subfaze **(confirmed: A)**

`f32`/`f64` la frontieră în **F40b** (cu `float/X`), nu doar F40e.

#### D326 — `atom_number/2` **(confirmed: amânat F40d)**

Neschimbat — livrare F40d.

### Fișiere țintă (draft)

| Fișier | Rol |
| ------ | --- |
| `logic-assembler.js` | Literale float; `parseMutationTerm` + `float` bindType; respinge `number/q*`/`number/fp16` |
| `logic-engine.js` | Kind `float`, unify, `float/1`, `is/2` reale (F40c) |
| `logic-float-formats.js` (nou?) | Codec `float/<fmt>` ↔ IEEE — bridge `LogTScriptNumericFormats` |
| `parser.js` | Query binding `float/<fmt>` |
| `components/logic.js` | Pin `float`, validare width |
| `doc/logic-value-types.md`, `logic-query-exec.md`, `logic-builtins.md` | EN + `logts-play` |

### Rezumat decizii F40 (partial)

| ID | Subiect | Decizie |
| -- | ------- | ------- |
| **D316** | Kind float | **A** — kind nou + `float/1` |
| **D317** | Frontieră fp/q | **B** — `float/<fmt>`; `number/fp16|q*` → eroare |
| **D318** | `is/2` reale | **A** — MVP F40c; extins **F42✅** |
| **D319** | Default `float` | **A** — `float` = `float/X`; packed list **32-bit f32** |
| **D320** | — | absorbit în D319 |
| **D321** | Tag-uri `f32`/`f64` | **A** — confirmed 2026-08-25 |
| **D322** | int vs float unify | **A** — fail |
...
**Out of scope F41:** literale `1.5` în sursa LogTScript non-logic; `is/2` extins → **F42✅**; packed vector/matrix SIMD nou.
...
## Faza 42 — `is/2` extins + **random float** **2+m** **(completed)**
...
### Problemă (rezolvată — livrat F42)

| Topic | Comportament azi |
| ----- | ---------------- |
| **`is/2` MVP** | ✅ F40c — `+ - * /`, promovare int→float, `/0` fail, int `/` trunc |
| **Operatori extinși** | ✅ F42a — `**`, `mod`, `rem`, `//` |
| **Funcții în expr** | ✅ F42b–c — `abs`, `sqrt`, `floor`, …, `min`, `max` |
| **`call(is/2)`** | ✅ F42d |
| **`random/1`** | ✅ F42e — float `[0.0, 1.0)` |
| **`random_between/3`** | ✅ F34 int + **F42e** float |
| **Gramatică expr** | ✅ F42 — precedență completă + funcții în expr |
...
| ------- | -------- | ------------------ | ------------- |
| **F42a** | Operatori **`**`**, **`mod`**, **`rem`**, **`//`** | **4386–4393✅** |
| **F42b** | Funcții unare incl. **`sqrt`** | **4394–4397✅** |
| **F42c** | **`min`/`max`** binare | **4398–4399✅** |
| **F42d** | **`call(is/2)`** meta-call | **4400–4401✅** |
| **F42e** | **`random/1`** + **`random_between`** float + doc | **4402–4410✅** |
...
### Decizii **(D332–D340)** **(confirmed user 2026-08-26)**
...
### Fișiere atinse (checklist implementare — **livrat**)
