...
todos:
  ...
  - id: 1.3-sock-parse-stream
    ...
    status: pending
  - id: 1.3-tests
    content: "Teste 2480–2486: peek, consume, tranzacție eroare, wave incremental, fără payload wire copy"
    status: pending
  - id: 1.3-doc
...
- Când `BITSIZE(rx) < 20` → handler **nu rulează** (guard) → aștepți append.
- Când vin biți noi → `on:1` re-evaluează → parse consume header.
- Restul rămâne în `rx` pentru următorul ciclu / alt `.proto`.

**Underflow la `{ data << rx }`:** eroare **tranzacție** — zero tăiere parțială (identic cu `save`/`restore` din `ParseStream`). Nu „skip silent” în 1.3 — Wave folosește guard `BITSIZE >= needed`.
...
- [`sock.md`](../../v0_3_2/doc/sock.md) — secțiune „Protocol + sock”: tabel `=` vs `<<`, anti-pattern payload→wire, pattern Wave.
- [`protocol-parse.md`](../../v0_3_2/doc/protocol-parse.md) — param `data` poate fi sock; `<<` = consume.
...
| ID | Scenariu |
|----|----------|
| 2480 | `{ data << rx }` — header consume; `BITSIZE(rx)` scade corect |
| 2481 | `{ data = rx }` — peek; după parse reușit, `BITSIZE(rx)` **neschimbat** |
| 2482 | literal mismatch cu `<<` → tranzacție; sock nemodificat |
| 2483 | underflow cu `<<` → eroare; sock nemodificat |
| 2484 | parseView `show(wire:field)` după consume parțial |
| 2485 | wave: append incremental + `on:1, BITSIZE >= N` + `{ data << rx }` |
| 2486 | wave: fără copiere payload în wire mare (verifică sock-only tail) |
