...
todos:
  - id: core-boolean-minimize
    ...
    status: pending
  - id: core-boolean-lut
    content: "core/boolean-lut.js — discover inputs multi-bit, lutOfGenerate, exprOfLutGenerate (expand A Nb → A.n), formatExprShort + formatExprStandard"
    status: pending
  - id: parse-exec
...
```text
sum(widths) === lutAddrBits   // lutAddrBits = ceil(log2(length))
```
...
| `exprOfLutGenerate(lutInst, varSpecs)` | validare sumă biți, QM per output bit, **2 linii** output |
